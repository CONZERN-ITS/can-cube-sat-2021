...
board_csv_files = [("brd_" + get_table_name(x), x,) for x in collect_csvs("onboard")]
ground_csv_files =[("gnd_" + get_table_name(x), x,) for x in collect_csvs("ground")]

db_file = "its-telemetry-ascent.sqlite"
try:    
    os.remove(db_file)
