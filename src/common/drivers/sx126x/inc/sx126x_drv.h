...
typedef struct sx126x_drv_lora_rx_timeout_cfg_t
{
	...
	/*! Если не равен нулю, то радио игнорирует таймаут в миллисекундах и работает
		только по этому таймауту в символах */
	uint8_t lora_symb_timeout;
	//! Разрешает остановку rx таймаут таймера по получению символов преамбулы
	/*! Если это не разрешено, то радио останавливает таймер только по заголовку пакета */
