...
int sx126x_api_set_sleep(sx126x_plt_t * plt, uint8_t sleep_flags)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_SLEEP, &sleep_flags, sizeof(sleep_flags));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_standby(sx126x_plt_t * plt, sx126x_standby_mode_t mode)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_STANDBY, &mode_value, sizeof(mode_value));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_FS, NULL, 0);
	return rc;
}
...
int sx126x_api_set_tx(sx126x_plt_t * plt, uint32_t timeout_ms)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_TX, timeout_bytes, sizeof(timeout_bytes));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_rx(sx126x_plt_t * plt, uint32_t timeout_ms)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_RX, timeout_bytes, sizeof(timeout_bytes));
	if (0 != rc)
		return rc;
...
int sx126x_api_stop_rx_timer_on_preamble(sx126x_plt_t * plt, bool enable)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_STOPRXTIMERONPREAMBLE, &arg, sizeof(arg));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_rx_duty_cycle(sx126x_plt_t * plt, uint32_t rx_time, uint32_t sleep_time)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_RXDUTYCYCLE, args, sizeof(args));
	if (0 !=  rc)
		return rc;
...
int sx126x_api_set_cad(sx126x_plt_t * plt)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_CAD, NULL, 0);
	if (0 != rc)
		return rc;
...
int sx126x_api_set_tx_continous_wave(sx126x_plt_t * plt)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_TXCONTINUOUSWAVE, NULL, 0);
	if (0 != rc)
		return rc;
...
int sx126x_api_set_tx_continous_preamble(sx126x_plt_t * plt)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_TXCONTINUOUSPREAMBLE, NULL, 0);
	if (0 != rc)
		return rc;
...
int sx126x_api_set_regulator_mode(sx126x_plt_t * plt, sx126x_regulator_t mode)
{
	...
	uint8_t arg = mode;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_REGULATORMODE, &arg, sizeof(arg));
	if (0 != rc)
		return rc;
...
int sx126x_api_calibrate(sx126x_plt_t * plt, uint8_t calib_flags)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_CALIBRATE, &calib_flags, sizeof(calib_flags));
	if (0 !=  rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_CALIBRATEIMAGE, args, SX126X_API_CALIBRATE_IMAGE_ARG_LEN);
	if (0 != rc)
		return rc;
...
)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_PACONFIG, args, sizeof(args));
	if (0 != rc)
		return 0;
...
int sx126x_api_set_rxtx_fallback_mode(sx126x_plt_t * plt, sx126x_fallback_mode_t mode)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_RXTXFALLBACKMODE, &arg, sizeof(arg));
	if (0 != rc)
		return rc;
...
)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_CFG_DIOIRQ, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_get_irq_status(sx126x_plt_t * plt, uint16_t * irq_status)
{
	...
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_GET_IRQSTATUS, NULL, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_clear_irq_status(sx126x_plt_t * plt, uint16_t clear_mask)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_CLR_IRQSTATUS, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_dio2_as_rf_switch(sx126x_plt_t * plt, bool enable)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_RFSWITCHMODE, &arg, sizeof(arg));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_dio3_as_tcxo_ctl(sx126x_plt_t * plt, sx126x_tcxo_v_t tcxo_voltage, uint32_t timeout_ms)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_TCXOMODE, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_rf_frequency(sx126x_plt_t * plt, uint32_t frequency)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_RFFREQUENCY, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_packet_type(sx126x_plt_t * plt, sx126x_packet_type_t packet_type)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_PACKETTYPE, &arg, sizeof(arg));
	if (0 != rc)
		return rc;
...
int sx126x_api_get_packet_type(sx126x_plt_t * plt, sx126x_packet_type_t * packet_type)
{
	...
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_SET_PACKETTYPE, NULL, &arg, sizeof(arg));
	if (0 != rc)
		return rc;
...
int sx126x_api_set_tx_params(sx126x_plt_t * plt, int8_t power, sx126x_pa_ramp_time_t pa_ramp_time)
{
	...
	const uint8_t args[2] = { power, pa_ramp_time }; // FIXME: проверить каст знака в беззнак
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_TXPARAMS, args, sizeof(args));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_MODULATIONPARAMS, raw_args, raw_args_size);
	return rc;
}
...
int sx126x_api_set_gfsk_modulation_params(sx126x_plt_t * plt, const sx126x_gfsk_modulation_params_t * params)
{
	...
	rc = sx126x_api_set_modulation_params(plt, args, sizeof(args));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_PACKETPARAMS, raw_args, raw_args_size);
	return rc;
}
...
int sx126x_api_set_gfsk_packet_params(sx126x_plt_t * plt, const sx126x_gfsk_packet_params_t * params)
{
	...
	rc = sx126x_api_set_packet_params(plt, args, sizeof(args));
	if (0 != rc)
		return rc;
...
)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_CADPARAMS, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_buffer_base_address(sx126x_plt_t * plt, uint8_t tx_base_addr, uint8_t rx_base_addr)
{
	...
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_BUFFERBASEADDRES, args, sizeof(args));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_SET_LORASYMBTIMEOUT, &symb_num, sizeof(symb_num));
	if (0 != rc)
		return rc;
...
int sx126x_api_get_status(sx126x_plt_t * plt, sx126x_status_t * status)
{
	...
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_GET_STATUS, &status_value, NULL, 0);
	if (0 != rc)
		return rc;
...
int sx126x_api_get_rx_buffer_status(sx126x_plt_t * plt, uint8_t * payload_size, uint8_t * payload_offset)
{
	...
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_GET_RXBUFFERSTATUS, NULL, data, sizeof(data));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_read(
			plt, SX126X_CMD_GET_PACKETSTATUS, NULL, raw_data_buffer, SX126X_API_PACKET_STATUS_ARG_LEN
	);
...
int sx126x_api_get_gfsk_packet_status(sx126x_plt_t * plt, sx126x_gfsk_packet_status_t * status)
{
	...
	rc = sx126x_api_get_packet_status(plt, raw);
	if (0 != rc)
		return rc;
...
int sx126x_api_get_rssi_inst(sx126x_plt_t * plt, int8_t * rssi)
{
	...
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_GET_RSSIINST, NULL, args, sizeof(args));
	if (0 != rc)
		return rc;
...
int sx126x_api_get_stats(sx126x_plt_t * plt, sx126x_stats_t * stats)
{
	...
	uint8_t raw[6];
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_GET_STATS, NULL, raw, sizeof(raw));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_RESET_STATS, NULL, 0);
	if (0 != rc)
		return rc;
...
int sx126x_get_device_errors(sx126x_plt_t * plt, uint16_t device_errors)
{
	int rc;
	uint8_t raw[2];
	rc = sx126x_plt_cmd_read(plt, SX126X_CMD_GET_ERROR, NULL, raw, sizeof(raw));
	if (0 != rc)
		return rc;
...
{
	int rc;
	rc = sx126x_plt_cmd_write(plt, SX126X_CMD_CLR_ERROR, NULL, 0);
	if (0 != rc)
		return rc;
