...
static int _send_packet(sx126x_drv_t * radio)
{
	int rc;
...
static void _event_handler(sx126x_drv_t * drv, void * user_arg, sx126x_evt_kind_t kind, const sx126x_evt_arg_t * arg)
{
	...
	case SX126X_EVTKIND_RX_DONE:
		...
		{
			// Если мы ничего не получили - ничего и не отправляем
			// Продолжаем слушать
		...
		{
			// Если получили - достаем пакет и быстро отправляем его же в ответ
			rc = _fetch_packet(&_radio);
			if (0 != rc) break;

			rc = _send_packet(&_radio);
			if (0 != rc) break;
		}
		break;
...
int app_main(void)
{
	...
	rc = sx126x_drv_mode_rx(&_radio, 0);
	printf("first rx: %d\n", rc);
