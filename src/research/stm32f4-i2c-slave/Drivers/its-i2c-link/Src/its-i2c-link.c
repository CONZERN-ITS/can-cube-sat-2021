 ...
 */
static int _link_tx_send_packet(I2C_HandleTypeDef *hi2c, i2c_link_ctx_t * ctx)
{
 ...
 */
static int _link_tx_dispatch(I2C_HandleTypeDef *hi2c, i2c_link_ctx_t * ctx)
{
    int rc = 0;
    switch (ctx->cur_cmd)
    {
    case I2C_LINK_CMD_GET_PACKET:
        rc = _link_tx_send_packet(hi2c, ctx);
        printf("tx disp packet %d\n", rc);
        break;

    case I2C_LINK_CMD_GET_SIZE:
        rc = _link_tx_send_size(hi2c, ctx);
        printf("tx disp size %d\n", rc);
        break;
...
static int _link_rx_recieve_packet(I2C_HandleTypeDef *hi2c, i2c_link_ctx_t * ctx)
{
 ...
 */
static int _link_rx_dispatch(I2C_HandleTypeDef *hi2c, i2c_link_ctx_t * ctx) {
    int rc = 0;

    switch (ctx->cur_cmd)
    {
    case I2C_LINK_CMD_SET_PACKET:
        rc = _link_rx_recieve_packet(hi2c, ctx);
        printf("rx disp pkt %d\n", rc);
        break;

    default:
        rc = _link_rx_recieve_cmd(hi2c, ctx);
        printf("rx disp cmd %d\n", rc);
        break;
    }
    if (0 != rc)
        return rc;
...
// В старых версиях HAL, в случае наступления ошибки от которой I2C модуль сам не поднимается
// HAL блокировал перефирию загоняя её резет
// Новые версии так не делают, поэтому будем делать это сами, ручками
void _lock_on_failure(i2c_link_ctx_t * ctx)
{
	ctx->iface.hi2c->Instance->CR1 |= I2C_CR1_SWRST;
}


// В ряде случае HAL забрасывает I2C перефирию в состояние SWRST
// (новые версии так не делают к сожалению)
...
{
    HAL_StatusTypeDef hal_rc;
    I2C_HandleTypeDef * const hi2c = ctx->iface.hi2c;

    if ((hi2c->Instance->CR1 & I2C_CR1_SWRST) != 0)
    {
...
int its_i2c_link_start()
{
    ...
    HAL_StatusTypeDef hal_rc = HAL_I2C_EnableListen_IT(ctx->iface.hi2c);
...
int its_i2c_link_reset(void)
{
    ...
    // Рубим прямо по жести и прямо сразу
    _lock_on_failure(ctx);
    _antihang(ctx);
...
void HAL_I2C_AddrCallback(I2C_HandleTypeDef *hi2c, uint8_t transfer_direction,
    ...
    case I2C_DIRECTION_RECEIVE:
        rc = _link_tx_dispatch(hi2c, ctx);
        break;

    case I2C_DIRECTION_TRANSMIT:
        rc = _link_rx_dispatch(hi2c, ctx);
        break;
    ...
    if (0 != rc)
    {
        // Если мы не смогли отреагировать на запрос ведущего
        // Это значит что мы сейчас сидим и затягиваем клок
        // и блокируем работу по шине полностью
        // С этим нужно делать что-то решительное.
        // В идеале нужно полностью глушить I2C периферию
        // но поскольку мы сейчас в прерывании
        // активной деятельности разводить не будем
        // Просто загоним модуль в резет
        _lock_on_failure(ctx);
    }
}
...
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef *hi2c)
{
    ...
    case I2C_LINK_STATE_RX:
        if (error == HAL_I2C_ERROR_AF)
        {
            ...
            // Или даже не на последний, так как мы читаем больше чем запросили
            _link_rx_complete(hi2c, ctx);
        }
        else
        {
    ...
    {
        // От такой ошибки модуль сам не оправляется
        // Старые версии хала в такой ситуации загоняли модуль в резет. Мы сделаем так же
        hi2c->Instance->CR1 |= I2C_CR1_SWRST;
        // По этому флагу и будем доставать модуль из резета
        // так как ему в любом случае нужна реинициализация
    }
}
